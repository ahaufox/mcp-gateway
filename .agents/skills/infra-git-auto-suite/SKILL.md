---
name: infra-git-auto-suite
description: Git 自动化套件 - 通过单条指令完成从代码变更预览到 PR 创建的全过程，包含强制代码审查、安全检查，以及暂存区为空时按变更分类与按类验证的智能选择性提交。
---
...
2. **严格禁止**输出与该技能目标无关的代码。
3. **必须**保证输出内容符合系统设定的高可用与安全性标准。
4. **严格禁止** `git push --force`、`git push --force-with-lease`、`reset --hard`、`clean -f`、向 `main` / `master` 等受保护分支直接推送等高风险操作，除非用户在当次会话中显式确认。
5. **严格禁止**绕过项目鉴权或绕过代码审查与本地静态检查直接 commit。
...
3. 按 `类型 + 作用域` 维度对变更进行**聚类**（详见下文「分类规则」）。
...
2. 验证耗时命令（`build`、`pytest`、`tsc`）默认走 `light`/`standard` 不会跑；只有 `strict` 档位才会触发；`--full-verify` 相当于**强制 strict**。
3. **任何验证失败**：
   - 整组保留在工作区，**不暂存**、**不提交**。
   - 记录到 `git-auto-suite-report.json` 的 `failed_buckets` 字段，包含失败命令、退出码、错误摘要与**触发的复杂度维度**。
4. **任何验证成功**：
   - 进入 B.4 进行选择性提交。
5. **复杂度证据可追溯**：报告中必须保留每个分类的 `complexity` 字段（`score`、`level`、`signals`），方便事后审查。
...
2. **生成本组提交信息**：
   ...
   - Body 列出本组修改的核心文件 + 验证通过项，例如：
     ...
     - 验证通过: ruff / mypy
     ```
3. **执行 `git commit -m "<title>" -m "<body>"`**，捕获 pre-commit hook 错误。
4. **提交失败处理**：
   - 撤回本组暂存：`git reset HEAD -- <files>`（默认行为）。
   - 整组归入 `failed_buckets`，进入 B.5 修复循环。
5. **多分类并行性**：默认**串行**提交，避免多组 commit 互相干扰；如用户显式传入 `--parallel`，可并发提交（仍需每组独立暂存/独立 commit）。
...
2. **审查阻断**：模式 A 中如审查不通过，必须修复问题后重新执行。
3. **分支保护**：不要直接向受保护的分支（如 `main`、`master`）推送，应通过 PR 流程。
4. **冲突处理**：如果推送时遇到冲突，需要先拉取远程变更并解决冲突。
5. **大文件警告**：如果检测到大型二进制文件（>10MB），提醒用户使用 Git LFS。
6. **取消强制 act 检查**：鉴于本地 `act` 极其耗时，不强制在提交流程中启动 `act` 模拟完整 CI。请优先使用本地轻量工具（如 `ruff check`、`mypy`、`npm run lint`）进行快速静态检查与类型合规性验证。
7. **失败透明**：任何验证失败、commit 失败、push 失败都必须**显式报告**原始错误，禁止静默吞错。
8. **回滚能力**：执行 commit 前对每组保存 `git rev-parse HEAD` 到报告，必要时可一键 `git reset --mixed <pre_commit_sha>` 回退。
