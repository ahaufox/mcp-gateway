...
本规范为全栈唯一的 UI 视觉与样式基准。所有 UI 开发必须严格遵循以下硬性约束。
...
### 5.4 消息提示 (Toast)
- **全局统一位置**：所有 Toast 提示必须居中且置于屏幕上方（即 `top-center` 定位，距离顶部合理的间距），禁止放置在屏幕四角或底部。
- **动态蒙层原则**：
  - **轻提示 (Non-blocking)**：用于常规的“保存成功”、“操作完成”等，不需要蒙层，不能阻断用户其他区域的点击操作。
  - **阻断提示 (Blocking)**：用于“即将跳转”、“处理中”等明确不允许用户中途介入的状态，必须附带一层透明或半透明的蒙层（Mask），防止误触。
