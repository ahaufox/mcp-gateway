...
- **前端项目 (`web/`)**：`src/` 下的 Hooks（`hooks/`）、组件（`components/`）、Pages（`pages/`）、Store、Utils 以及 Context 定义。

**豁免范围**：纯配置文件（如 `vite.config.ts`、`tsconfig.json`、`eslint.config.js`）、自动生成文件、入口文件（`main.tsx`、`index.tsx`）、类型声明文件（`*.d.ts`）。
 ...
 *   - 格式参考：
 *     - 原内部 useState (mcpList) → 全部委托给 useMcpStore
 *     - config 类型由 any 升级为 McpConfig | null
 */
```text
...
| 字段 | 说明 | 示例 |
|------|------|------|
| **标题行** | 文件名或导出名 + 破折号 + 一句话简述 | `useMcpConverter — MCP 协议转换核心 Hook` |
| **职责** | 描述该文件负责什么、对外暴露什么 | `职责：封装将 Swagger/OpenAPI 定义转换为 MCP 工具声明的核心转换逻辑，以及本地加载与校验操作。` |
